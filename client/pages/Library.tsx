...
import { Play, Check, Pause, Clock, Star, BarChart3 } from 'lucide-react';
...
export default function Library() {
  ...
  const filteredLibrary = userLibrary.filter(anime => anime.userStatus === activeTab);
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3 sm:gap-4 mb-6 sm:mb-8">
        <GlassCard className="p-3 sm:p-4 text-center">
          <div className="text-lg sm:text-2xl font-bold text-success">{libraryStats.watching}</div>
          <div className="text-xs sm:text-sm text-muted-foreground">Watching</div>
        </GlassCard>
        <GlassCard className="p-3 sm:p-4 text-center">
          <div className="text-lg sm:text-2xl font-bold text-primary">{libraryStats.completed}</div>
          <div className="text-xs sm:text-sm text-muted-foreground">Completed</div>
        </GlassCard>
        <GlassCard className="p-3 sm:p-4 text-center">
          <div className="text-lg sm:text-2xl font-bold text-warning">{libraryStats.paused}</div>
          <div className="text-xs sm:text-sm text-muted-foreground">On Hold</div>
        </GlassCard>
        <GlassCard className="p-3 sm:p-4 text-center">
          <div className="text-lg sm:text-2xl font-bold text-secondary">{libraryStats.planning}</div>
          <div className="text-xs sm:text-sm text-muted-foreground">Planning</div>
        </GlassCard>
        <GlassCard className="p-3 sm:p-4 text-center">
          <div className="text-lg sm:text-2xl font-bold gradient-text-subtle">{libraryStats.totalEpisodes}</div>
      ...
      <GlassCard className="p-4 sm:p-6">
        <Tabs value={activeTab} onValueChange={(value) => setActiveTab(value as UserStatus)}>
          <TabsList className="grid w-full grid-cols-4 mb-4 sm:mb-6">
            {(['watching', 'completed', 'paused', 'planning'] as UserStatus[]).map((status) => {
              ...
              return (
                ...
                >
                  ...
                  <span className="hidden sm:inline">{getUserStatusLabel(status)}</span>
                  <span className="sm:hidden text-xs">
                    {status === 'watching' ? 'Watch' :
                     status === 'completed' ? 'Done' :
                     status === 'paused' ? 'Hold' : 'Plan'}
                  </span>
              ...
              {filteredLibrary.length === 0 ? (
                <div className="text-center py-12">
                  ...
                  <p className="text-muted-foreground mb-4">
                    {status === 'watching' && "Start watching some anime to see them here!"}
                    {status === 'completed' && "Complete some anime to build your collection!"}
                    {status === 'paused' && "Anime you put on hold will appear here."}
                    {status === 'planning' && "Add anime to your plan to watch list!"}
                  </p>
                  <Button asChild>
                    <Link to="/catalog">
                      Browse Catalog
                      ...
                      <div className="space-y-2">
                        {filteredLibrary.map((anime) => (
                          <div key={anime.id} className="flex items-center justify-between text-sm">
                            <span className="truncate flex-1 mr-4">{anime.title}</span>
